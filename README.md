...
В LLM передаются только AI-digest строки, которые уже сматчились через `ai_product_mapping.xlsx`. DD-метрики, индекс и несматченные продукты в prompt не отправляются.
