...
По умолчанию в расчёт входит история с 1 февраля 2026 года; более ранние
тикеты, отменённые тикеты и строки без `Issue key` исключаются. Границу истории
можно изменить параметром `--history-start`. Структура бэклога строится по
месяцу `Created`: каждая задача учитывается ровно один раз, в месяце создания.
В публичный JSON не выгружаются ключи тикетов и данные сотрудников.
...
excludedMissingIssueKey + excludedBeforeHistoryStart` для каждой команды и проверки
