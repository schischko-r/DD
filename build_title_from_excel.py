      ...
      return `
        <div class="unit-row ${isFirst ? 'first' : ''}">
          <div class="unit-name">
            ...
            <b>${esc(unit)}</b>
            <span>${rows.length} ${pluralEntity(rows.length)}</span>
          </div>
          <div class="unit-avg">средний Data-Driven Index <b style="color:${groupTheme(avgGroup).text}">${avg}%</b></div>
        </div>
      `;
    }
