...
export {allocateIndexUplifts, blockPercent, difficultyMeta, filterCampaigningLinks, filterDraftLinks, filterInapplicableMetricGroups, filterInapplicableMetricSubgroups, filterMetricRelevantLinks, filterMetricsForBlock, groupFor, hasMetricDeviations, hasPilotCampaignSkill, inapplicableMetricLabel, isCampaigningRelevant, isCrossSellDigitallyConfirmed, isDdIndexMetric, isDraftsRelevant, isInformationalMetric, isTbdMetric, metricDomId, metricSkillLinks, percent, radarBlockPercent, scoreFor, summarizeRecommendationUplifts, teamHelpAudience};
...
export function ApplicableRadarShape({points, ...props}) {
  const applicablePoints = (points || []).filter((point) => point.value != null);
  return <Polygon {...props} points={applicablePoints} />;
}

export function ApplicableRadarDot({value, dotFill, dotRadius, ...props}) {
  if (value == null) return null;
  return <Dot {...props} fill={dotFill || props.fill} r={dotRadius || props.r} />;
}
