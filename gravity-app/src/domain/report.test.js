...
test('churn help audience distinguishes products and segment types', () => {
  assert.equal(churnHelpAudience({type: 'Продукт', name: 'ОСАГО'}), 'product');
  assert.equal(churnHelpAudience({type: 'Сегмент', name: 'Дети'}), 'age');
  assert.equal(churnHelpAudience({type: 'Сегмент', name: 'PB'}), 'income');
  assert.equal(churnHelpAudience({type: 'Сегмент', name: 'Другой сегмент'}), 'segment');
  assert.equal(churnHelpAudience({type: 'Канал', name: 'СБОЛ'}), '');
});
