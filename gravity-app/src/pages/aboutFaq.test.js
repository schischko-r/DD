...
test('FAQ deep-link survives the view transition and stacks cleanly on mobile', () => {
  ...
  assert.match(stylesSource, /\.about-faq-item \.g-disclosure__trigger\s*\{[^}]*width:\s*100%/s);
  assert.match(stylesSource, /\.about-faq-item \.g-disclosure__content\s*\{[^}]*padding:\s*10px calc\(var\(--about-faq-inline\) \+ var\(--about-faq-arrow-space\)\) 20px calc\(var\(--about-faq-inline\) \+ var\(--about-faq-number\) \+ var\(--about-faq-gap\)\)/s);
...
test('FAQ reuses the methodology surface, borders, and brand interaction tokens', () => {
  assert.doesNotMatch(aboutPageSource, /\{DD_FAQ\.length\} вопросов/);
  assert.doesNotMatch(stylesSource, /\.about-faq-heading > \.g-label/);
