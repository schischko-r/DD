...
test('team profile renders the roadmap uplift directly below the next-level copy', () => {
  ...
  assert.match(roadmapSource, /Исходя из дорожной карты продукта ожидается аплифт:/);
  assert.match(roadmapSource, /<Button className="index-roadmap-uplift-trigger" view="flat-success" size="s" type="button" aria-haspopup="dialog"/);
  assert.match(roadmapSource, /onClick=\{\(\) => setOpen\(true\)\}/);
  assert.match(roadmapSource, />\+ \{upliftLabel\} п\.\u043f\.<\/Button>/);
...
test('roadmap dialog contains one semantic table with the exact five-column header', () => {
  assert.match(roadmapSource, /<section className="index-roadmap-uplift" aria-label="Прогноз по дорожной карте">/);
  assert.match(roadmapSource, /aria-label=\{`Открыть детализацию дорожной карты: плюс \$\{upliftLabel\} процентных пункта`\}/);
  ...
  assert.match(roadmapSource, /<thead>\s*<tr>\s*<th scope="col">Блок для развития<\/th>\s*<th scope="col">Квартал<\/th>\s*<th scope="col">Срок<\/th>\s*<th scope="col">Планируемое мероприятие<\/th>\s*<th scope="col">Ожидание прироста индекса DD, п\.п\.<\/th>\s*<\/tr>\s*<\/thead>/);
...
test('roadmap grouped cells use accessible rowgroup headers and computed rowSpan values', () => {
  ...
  assert.match(roadmapSource, /row\.dueDateRowSpan > 0 && <th className="roadmap-group-cell roadmap-due-date-cell" scope="rowgroup" rowSpan=\{row\.dueDateRowSpan\}>\{roadmapDate\(row\.dueDateLabel\)\}<\/th>/);
});
...
test('roadmap UI uses tokenized 4px borders and keeps table overflow local on mobile', () => {
  ...
  assert.match(stylesSource, /\.index-roadmap-uplift-trigger \.g-button__text\s*\{[^}]*var\(--g-color-text-positive-heavy\);[^}]*font-weight:\s*700;/s);
