...
test('recommendation table uses Gravity UI and is the final backlog analytics section', () => {
  ...
  assert.match(pageSource, /const columns = buildScenarioRecommendationColumns\(\)[\s\S]*?<Table[\s\S]*?columns=\{columns\}[\s\S]*?data=\{rows\}[\s\S]*?wordWrap/);
  assert.match(pageSource, /name: 'Направление'[\s\S]*?name: 'Сценарий'[\s\S]*?name: '% времени'[\s\S]*?name: 'TTM · топ-25%'[\s\S]*?name: 'TTM команды'[\s\S]*?name: 'Описание'[\s\S]*?name: 'Рекомендация тимлиду'/);
  ...
  assert.match(stylesSource, /\.dd-scenario-recommendations-table\s*\{\s*min-width:\s*1320px;/);
...
test('recommendations consistently retain inline resources and access modals', () => {
  ...
  assert.equal((pageSource.match(/<RecommendationCell><RecommendationCopy item=\{item\} \/><\/RecommendationCell>/g) || []).length, 2);
...
test('team TTM attention requires an actual top-25 benchmark', () => {
